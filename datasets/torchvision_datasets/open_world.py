...
ALL_CLASS_NAMES = tuple(itertools.chain(T1_CLASS_NAMES, T2_CLASS_NAMES, T3_CLASS_NAMES, T4_CLASS_NAMES, UNK_CLASS))
...
class OWDetection(VisionDataset):
    ...
    def __init__(self,
        ...
        for image_set in image_sets:
            ...
            start_idx = len(self.imgids)
            current_imgids = list(range(start_idx, start_idx + len(file_names)))
            ...
            self.imgids.extend(current_imgids)
            # self.imgids.extend(file_names)
            self.imgid2annotations.update(dict(zip(current_imgids, self.annotations[-len(file_names):])))
